    ...
    const DynamicPluginTensorDesc* in, int nbInputs, const DynamicPluginTensorDesc* out, int nbOutputs) noexcept
{
    try
    {
        ...
        PLUGIN_ASSERT(in[2].desc.dims.nbDims == 3 || in[2].desc.dims.nbDims == 4);
        if (in[2].desc.dims.nbDims == 3)
        {
            PLUGIN_ASSERT(in[2].desc.dims.d[2] == 10);
        }
        else
        {
            PLUGIN_ASSERT(in[2].desc.dims.d[2] == 1);
            PLUGIN_ASSERT(in[2].desc.dims.d[3] == 10);
        }
