...
template <typename Trois, typename Tfeat>
__global__ void roiAlign_kernel_new(const int imageSize, const int featureCount, const int roiCount,
...
cudaError_t roiAlignNew(const cudaStream_t stream, const int batchSize, const int imageSize,
    ...
    const void* rois, const void* layers, const xy_t layerDims, void* pooled, const int poolDims)
{
    ...
    roiAlign_kernel_new<<<blocks, threads, 0, stream>>>(imageSize, featureCount, roiCount, transformCoords,
